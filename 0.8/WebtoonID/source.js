...
var source = (() => {
  ...
  var require_id = __commonJS({
    "node_modules/moment/locale/id.js"(exports, module) {
      (function(global, factory) {
        ...
        "use strict";
        var id = moment3.defineLocale("id", {
  ...
  // src/WebtoonID/WebtoonID.ts
  var WebtoonID_exports = {};
  __export(WebtoonID_exports, {
    WebtoonID: () => WebtoonID,
    WebtoonIDInfo: () => WebtoonIDInfo
  });
  ...
  var WebtoonParser = class {
    constructor(dateFormat, language, BASE_URL2, MOBILE_URL2) {
    ...
    parseDetails($, mangaId) {
      ...
      const [image, title] = mangaId.startsWith("canvas") ? [this.parseCanvasDetailsThumbnail($), detailElement.find("h3").text().trim()] : [this.parseDetailsThumbnail($), detailElement.find("h1").text().trim()];
    ...
    parseChaptersList($) {
      return $("ul#_episodeList > li[id*=episode]").toArray().map((elem) => this.parseChapter($(elem)));
    }
    parseChapter(elem) {
    ...
    parsePopularTitles($) {
      return $("div#content div.NE\\=a\\:tnt li a").toArray().filter((elem) => $(elem).find("p.subj")).map((elem) => this.parseMangaFromElement($(elem)));
    }
    ...
    parseCanvasRecommendedTitles($) {
      return $("#recommendArea li.rolling-item").toArray().map((elem) => this.parseCanvasFromRecommendedElement($(elem)));
    }
    ...
    parseCanvasPopularTitles($) {
      return $("div.challenge_lst li a").toArray().map((elem) => this.parseCanvasFromElement($(elem)));
    }
    ...
    parseSearchResults($, canvas_wanted) {
      const items = [];
      items.push(...$("#content > div.card_wrap.search li a.card_item").toArray().map((elem) => this.parseMangaFromElement($(elem))));
      if (canvas_wanted) {
        items.push(...$("#content > div.card_wrap.search li a.challenge_item").toArray().map((elem) => this.parseCanvasFromElement($(elem))));
      }
    ...
    parseGenres($) {
      return $("#content ul._genre li").toArray().map((elem) => this.parseTagFromElement($(elem)));
    }
    parseCanvasGenres($) {
      return $("#content ul.challenge li").toArray().filter((elem) => $(elem).attr("data-genre") && $(elem).attr("data-genre") !== "ALL").map((elem) => this.parseCanvasTagFromElement($(elem)));
    }
    ...
    parseTagResults($) {
      return App.createPagedResults({
        results: $("#content > div.card_wrap ul.card_lst li a").toArray().map((elem) => this.parseMangaFromElement($(elem)))
      });
    }
  };

  // src/WebtoonSettings.ts
  var getCanvasWanted = async (stateManager) => {
    return await stateManager.retrieve("canvas_wanted") ?? false;
  };
  var setCanvasWanted = async (stateManager, value) => {
    await stateManager.store("canvas_wanted", value);
  };
  var configSettings = (stateManager) => {
          ...
          return [
            App.createDUISwitch({
              ...
              value: App.createDUIBinding({
                ...
                set: async (value) => {
                  await setCanvasWanted(stateManager, value);
                }
              })
            }),
            App.createDUIButton({
  ...
  var Webtoon = class {
    constructor(cheerio, LOCALE2, DATE_FORMAT2, LANGUAGE2, BASE_URL2, MOBILE_URL2, HAVE_TRENDING2) {
      this.cheerio = cheerio;
      this.LOCALE = LOCALE2;
      this.BASE_URL = BASE_URL2;
      this.MOBILE_URL = MOBILE_URL2;
      ...
      this.parser = new WebtoonParser(DATE_FORMAT2, LANGUAGE2, BASE_URL2, MOBILE_URL2);
      this.cookies = [
  ...
  import_moment2.default.updateLocale("id", { monthsShort: "Jan_Feb_Mar_Apr_Mei_Jun_Jul_Agu_Sep_Okt_Nov_Des".split("_") });
  var LOCALE = "id";
  var DATE_FORMAT = "YYYY MMM D";
  var LANGUAGE = "id";
  var BASE_URL = `${BASE_URL_XX}/${LOCALE}`;
  var MOBILE_URL = `${MOBILE_URL_XX}/${LOCALE}`;
  var VERSION = "0.0.0";
  var LANGUAGE_INFO = "Indonesian";
  var SOURCE_NAME = "WebtoonID";
  var HAVE_TRENDING = true;
  var SOURCE_TAGS = [{ text: "Indonesian", type: import_types2.BadgeColor.GREY }];
  var WebtoonIDInfo = {
    ...WebtoonBaseInfo,
    name: SOURCE_NAME,
